...
#define BALLOC_DEF_GROUPS_NR 65
...
static int balloc_group_info_load(struct m0_balloc *bal)
{
	...
	M0_LOG(M0_INFO, "Loading group info...bsb_groupcount : %llu", (unsigned long long)bal->cb_sb.bsb_groupcount);
                                  ...
                                  uint64_t              partition)
{
	...
	M0_LOG(M0_INFO, "Nihar group_write insert btree index normal index: %d endoff: %llu", it, (unsigned long long)ext.e_end);
	...
	val = (struct m0_buf)M0_BUF_INIT_PTR(&ext.e_start);
	it = ext.e_end / sb->bsb_groupsize;
	M0_LOG(M0_INFO, "Nihar group write insert btree index spare index: %d endoff : %llu", it, (unsigned long long)ext.e_end);
				...
				m0_bcount_t spare_blocks_per_group)
{
	...
	M0_LOG(M0_INFO, "Nihar Group Count = %"PRIu64, bal->cb_sb.bsb_groupcount);
...
M0_INTERNAL int m0_balloc_load_extents(struct m0_balloc *cb,
				       struct m0_balloc_group_info *grp)
{
	struct m0_be_btree	  *db_ext;// = &cb->cb_db_group_extents[grp->bgi_groupno];
	...
	M0_LOG(M0_INFO, "Nihar load index get btree index normal index: %d endoff: %llu", it, (unsigned long long)normal_range.e_end);
					  ...
					  struct m0_be_tx_credit *accum)
{
	...
	for (i = 0; i < BALLOC_DEF_GROUPS_NR; i++) {
		m0_be_btree_delete_credit(&bal->cb_db_group_extents[i], 1,
			M0_MEMBER_SIZE(struct m0_ext, e_start),
			M0_MEMBER_SIZE(struct m0_ext, e_end), &cred);
		m0_be_btree_insert_credit(&bal->cb_db_group_extents[i], 1,
			M0_MEMBER_SIZE(struct m0_ext, e_start),
			M0_MEMBER_SIZE(struct m0_ext, e_end), &cred);
		m0_be_btree_update_credit(&bal->cb_db_group_extents[i], 2,
			M0_MEMBER_SIZE(struct m0_ext, e_end), &cred);
	}
...
static int balloc_alloc_db_update(struct m0_balloc *motr, struct m0_be_tx *tx,
				  ...
				  struct m0_ext *cur)
{
	struct m0_be_btree          *db;// = &motr->cb_db_group_extents[grp->bgi_groupno];
	struct m0_buf                key;
	struct m0_buf                val;
	struct m0_lext              *le;
	struct m0_lext              *lcur;
